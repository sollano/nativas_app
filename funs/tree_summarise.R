...
tree_summarise <- function(df,  dbh, tree, .groups=NA, vwb=NA, vwob=NA){
  ...
  }else if(check_names(df,tree)==F){
    # Parar se algum nome nao existir, e avisar qual nome nao existe
    stop(check_names(df,tree, boolean=F), call.=F) 
  }else{
  ...
  }else if(check_names(df, vwb)==F){
    stop(check_names(df, vwb, boolean=F), call.=F)
  }
  ...
  }else if(check_names(df, vwob)==F){
    stop(check_names(df, vwob, boolean=F), call.=F)
  }
